 ...
 * @param b
 */
extern "C" __device__ void swapF(float* a, float* b)
{
   float c = *a;
   *a = *b;
   *b = c;
 ...
 * @param b
 */
extern "C" __device__ void swapI(int* a, int* b)
{
   int c = *a;
   *a = *b;
   *b = c;
...
extern "C" __device__ void siftDown(float *array, int start, int end)
{
   int root = start;
...
extern "C" __device__ void heapify(float *array, int n)
{
   int start = ((n-1) - 1) / 2;
 ...
 * @param n
 */
extern "C" __device__ void heapSort(float *array, int n)
{
   heapify(array, n);
 ...
 * @param extraList
 */
extern "C" __device__ void bitonicSortFF(int size, float* sortList, float* extraList)
{
 ...
 * @param extraList
 */
extern "C" __device__ void bitonicSortFI(int size, float* sortList, int* extraList)
{
   // initialize all variables
   int bsize = size/2;
   int ob,ib,i,dir,a,b,t;
